...
import live.itrip.jvmm.agent.util.*;
...
public final class AgentBootStrap {
    private static final String JVMM_CORE_SERVER_JAR = "jvmm-agent-core.jar";
    private static final String SERVER_MAIN_CLASS = "live.itrip.jvmm.agent.server.ServerBootstrap";
...
//        tryFindSpringClassLoader();
    ...
    /**
     * 代理载入 jvmm-agent-core-1.0.0.jar 包，期间会由新的ClassLoader载入这些jar包，对一些需要预加载的jar包，在attach前会搜索过滤出这些jar
     * 然后再由自定义的ClassLoader载入。
     * <p>
     * 如果 jvmm-agent-core-1.0.0.jar 未加载过，并且 server 未启动，将会执行：解析参数、下载jar依赖、共享日志jar包、加载jar包、启动服务、启动日志处理线程；
     * 如果 jvmm-agent-core-1.0.0.jar 已加载过，并且 server 未启动，将会执行：解析参数、启动服务、启动日志处理线程；
     * 如果 jvmm-agent-core-1.0.0.jar 已加载过，并且 server 已启动，操作将被禁止；
     *
     * @param args 参数
    ...
    private static synchronized void main(String args, final Instrumentation inst, String type) {
        ...
        if (serverJar.startsWith("http://") || serverJar.startsWith("https://")) {
            boolean loaded = FileUtil.readFileFromNet(serverJar, AppUtil.getDataPath(), JVMM_CORE_SERVER_JAR);
            if (loaded) {
                serverJarFile = new File(AppUtil.getDataPath(), JVMM_CORE_SERVER_JAR);
            } else {
        ...
        if (!serverJarFile.exists()) {
            LoggerUtil.warn(AgentBootStrap.class, "Can not found jvmm-agent-core-1.0.0.jar file from args: " + serverJar);

            //  如果从参数中未成功读取jar包，依次按以下路径去寻找jar包
            //  1. 目标程序根目录下的 jvmm-agent-core-1.0.0.jar 包
            //  2. agent的资源目录下的 jvmm-agent-core-1.0.0.jar 包

            LoggerUtil.info(AgentBootStrap.class, "Try to find jvmm-agent-core-1.0.0.jar file from target program directory.");
            serverJarFile = new File(AppUtil.getHomePath(), JVMM_CORE_SERVER_JAR);
            if (!serverJarFile.exists()) {
                LoggerUtil.warn(LoggerUtil.class, "Can not found jvmm-agent-core-1.0.0.jar file from target program directory.");

                LoggerUtil.info(LoggerUtil.class, "Try to find jvmm-agent-core-1.0.0.jar file from agent jar directory.");
                CodeSource codeSource = AgentBootStrap.class.getProtectionDomain().getCodeSource();
                if (codeSource != null) {
                    try {
                        File agentJarFile = new File(codeSource.getLocation().toURI().getSchemeSpecificPart());
                        serverJarFile = new File(agentJarFile.getParentFile(), JVMM_CORE_SERVER_JAR);
                        if (!serverJarFile.exists()) {
                            LoggerUtil.error(AgentBootStrap.class, "Can not found jvmm-agent-core-1.0.0.jar file from agent jar directory.");
                        }
                    } catch (Throwable e) {
                        LoggerUtil.error(AgentBootStrap.class, String.format("Can not found jvmm-agent-core-1.0.0.jar file from %s. %s", codeSource.getLocation(), e.getMessage()), e);
                    }
                }
            }
        }
    ...
    private static void runLoggerConsumer() {
        ...
        ThreadExecutor.execute(() -> {
            Thread.currentThread().setName("jvmm-logger");
...
//        thread.start();
     ...
     * 由Agent Server反射调用
     * <p>
     * 调用位置：live.itrip.jvmm.server.logger.DefaultLoggerAdaptor#publish(live.itrip.jvmm.agent.common.logger.LoggerEvent)
     * <p>
    ...
    /**
     * server模块中 live.itrip.jvmm.server.ServerBootstrap#stop() 调用
     */
